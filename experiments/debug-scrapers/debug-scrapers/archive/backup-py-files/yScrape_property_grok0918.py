...
import re  # Added for smart extraction patterns
...
class PropertyScraper:
    ...
    async def scrape_homes_com(self, page):
        ...
        try:
            ...
            if random.random() < 0.5:  # 50% chance keyboard enter
                ...
                # Mouse click on submit button if available
                submit_selectors = ['button[type="submit"]', '.search-button', '[data-testid="search-submit"]']
            ...
            await page.wait_for_timeout(5000 + random.uniform(0, 3000))  # Wait for results
    ...
    async def extract_homes_property_data(self, page):
        """Extract property data from Homes.com property page with smart content scanning"""
        try:
            await self.log_step("EXTRACT_START", "Starting smart data extraction", page)
            
            body_text = await self.wait_for_property_content(page)
            if not body_text:
                self.data["errors"].append("No page content for extraction")
                return
            ...
            # Primary patterns
            patterns = {
                "price": r'\$\d{1,3}(?:,\d{3})*(?:\s*-\s*\$\d{1,3}(?:,\d{3})*)?',
                "beds": r'\d+\s*beds?',
                "baths": r'\d+\s*baths?',
                "sqft": r'\d{1,3}(?:,\d{3})*\s*Sq\s*Ft'
            }
            
            for key, pattern in patterns.items():
                match = re.search(pattern, body_text)
                if match:
                    property_data[key] = match.group(0).strip()
                    await self.log_step("EXTRACT_MATCH", f"Extracted {key}: {property_data[key]}", page)
            
            await self.extract_additional_property_details(page, body_text, property_data)
            
            self.data["property_data"] = property_data
            self.data["found"] = len(property_data) >= 3
    ...
    async def extract_additional_property_details(self, page, body_text, property_data):
        """Extract additional details using patterns"""
        try:
            additional_patterns = {
                "year_built": r'Built in \d{4}',
                "taxes": r'Tax: \$\d{1,3}(?:,\d{3})*',
                "last_sold": r'Last Sold: (?:\w+ \d{1,2}, \d{4} for \$\d{1,3}(?:,\d{3})*|N/A)'
            }
            ...
            for key, pattern in additional_patterns.items():
                ...
                if match:
                    property_data[key] = match.group(0).strip()
                    await self.log_step("ADDITIONAL_MATCH", f"Extracted {key}: {property_data[key]}", page)
            
            # Photos extraction (original)
            photo_selectors = [
                '.property-photos img',
                '.listing-photos img',
                '.photo-gallery img',
            ...
            await self.log_step("ADDITIONAL_ERROR", f"Error extracting additional: {str(e)}", page)
            self.data["errors"].append(f"Additional extraction error: {str(e)}")
