#!/usr/bin/env python3
"""
Property Scraper with Visual Documentation
Re-assessment of scraping logic with comprehensive screenshots
"""
...
class PropertyScraper:
    def __init__(self, address, site):
        ...
        self.screenshots_dir = f"screenshots_{self.site}_{self.timestamp}"
        self.data = {
            "address": address,
            "site": site,
...
async def scrape_homes_com(self, page):
    ...
    try:
        # Step 1: Navigate to direct search URL for landing page success (biggest win per Claude/review—bypass homepage bots)
        search_url = f"https://www.homes.com/search?address={self.address.replace(' ', '+').replace(',', '%2C')}"  # Direct landing
        await self.log_step("STEP_1_INITIAL_LOAD", f"Loading Homes.com direct search: {search_url}", page)
        ...
        await page.wait_for_timeout(random.uniform(500, 1500))  # Variable pause
        await self.log_step("STEP_1_LOADED", "Homes.com direct search loaded", page)
        
        # Step 2: Find and interact with search box (if not direct property)
        ...
        # ... rest unchanged (results check/extract) ...
...
async def run(self):
    ...
    async with async_playwright() as p:
        # Launch with Firefox for better evasion (Claude's success—greens anti-bot on Homes.com/Zillow)
        browser = await p.firefox.launch(
            ...
            args=[
                ...
                f'--user-agent={random.choice(self.user_agents)}'  # Random UA
            ],
            proxy={
        ...
        # Verbose network logging for debug
        ...
        try:
            ...
            # Linger window for inspection (remove post-test)
            await asyncio.sleep(10000)  # 10s hold to see window
