...
class SmartPropertyExtractor:
    ...
    def __init__(self):
        ...
        self.extended_patterns = {
            ...
            # AVM Providers (capture both name and value for each)
            'avm_collateral_analytics': r'Collateral Analytics.*?\$[\d,]+',
            'avm_corelogic': r'CoreLogic.*?\$[\d,]+',
            'avm_first_american': r'First American.*?\$[\d,]+',
            'avm_housecanary': r'HouseCanary.*?\$[\d,]+',
            'avm_provider': r'(Collateral Analytics|CoreLogic|First American|HouseCanary)',
            'avm_value': r'(?:Collateral Analytics|CoreLogic|First American|HouseCanary).*?\$(\d{1,3}(?:,\d{3})*)',
